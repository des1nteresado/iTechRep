...
namespace ParseTask2
{
    ...
    public class StarWarsAPIClient
    {
        ...
        public StarWarsEntityListLim<StarshipLim> GetListStarShips(IEnumerable<string> urls)
        {
            StarWarsEntityListLim<StarshipLim> starList;
            starList = GetAsync<StarWarsEntityListLim<StarshipLim>>(urls.First()).Result;

            //for (int i = 2; i < urls.Count() + 1; i++)
            //{
            //    starList.results.AddRange(GetAsync<StarWarsEntityListLim<StarshipLim>>(urls.ElementAt(i)).Result.results);
            //}
            //foreach (var url in urls)
            //{
            //    //var test = GetAsync<StarWarsEntityListLim<StarshipLim>>(url).Result;
            //    //foreach (var t in test.results)
            //    //{
            //    //    Debug.WriteLine(t.name); // write all 37 
            //    //    //starList.results.Add(t);
            //    //}
            //    //starList.results.AddRange(test.results);

            //    starList = GetAsync<StarWarsEntityListLim<StarshipLim>>(url).Result;
            //}

            //Debug.WriteLine("TEST");
            //foreach (var star in starList.results)
            //{
            //    Debug.WriteLine(star.name);
            //}
            return starList;
        }
        ...
        public StarWarsEntityListLim<StarshipLim> GetAllStarship(string page = "1")
        {
            var url = string.Format("starships?page={0}", page);
            return GetAsync<StarWarsEntityListLim<StarshipLim>>(url).Result;
        }
